...
private:
    ...
    unordered_map<string, vector<int>>
        packetStore; // key -> packet [source, dest, timestamp]
    unordered_map<int, vector<int>>
        destTimestamps; // destination -> sorted timestamps
...
public:
    ...
    vector<int> forwardPacket() {
        if (packetStore.empty())
            return {}; // If there are no packets to forward, return an empty
                       // array.
        ...
        int dest = packet[1];
        destTimestamps[dest].erase(
            destTimestamps[dest].begin()); // remove earliest timestamp
    ...
    int getCount(int destination, int startTime, int endTime) {
        ...
        int i = lower_bound(begin(it->second), end(it->second), startTime) -
                begin(it->second); // log(size of vec)
        int j = upper_bound(begin(it->second), end(it->second), endTime) -
                begin(it->second); // log(size of vec)
